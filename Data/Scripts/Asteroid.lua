...
function Asteroid.spawnAsteroids(asteroidSeed)
    ...
    local safeX,safeY = math.floor(g_World.width*.5-4),math.floor(g_World.height*.5-4)
    local nMaxTries = numToSpawn*3
    ...
    for i=1,numToSpawn do
        ...
        if DFMath.overlaps(safeX,safeY,safeX+8,safeY+8,
                tileX,tileY,tileX+tModuleData.tileWidth,tileY+tModuleData.tileHeight) then
            i=i-1
        else
