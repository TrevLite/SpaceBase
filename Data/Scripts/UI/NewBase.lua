...
function m.create()
    ...
    function Ob:init()
        ...
        self.rSidebarLeft = self:getTemplateElement('SideBarLeftTile1')
        self.rSidebarRight = self:getTemplateElement('SideBarRightTile1')
    ...
    function Ob:onFinger(touch, x, y, props)
        ...
        if touch.eventType == DFInput.TOUCH_UP and self.currentState == 'Initial' then
            ...
            --[[
            -- just some debug code to let you view event data while clicking on the world map.
            GameRules.tLandingZone = self.tLandingZone
            if self.tLandingZone and self.tLandingZone.x then
    ...
    function Ob:setCursor(wx,wy)
        ...
        if (dataMapX == 12 and dataMapY == 34) or (dataMapX == 13 and dataMapY == 34) then
...
--        EventController.fromSaveData({})
    ...
    function Ob:isOnMap(x, y)
        local _,_,bValid = self:worldToMap(x,y)
        return bValid
    end
    
    function Ob:worldToMap(x,y)
        local mw,mh = self.rMap:getDims()
        local msw,msh=self.rMap:getScl()
        mw,mh=mw*msw,mh*msh
        local mx,my = self.rMap:getLoc()
        local parentX,parentY = self.rMap:getAttrLink(MOAITransform.INHERIT_TRANSFORM):getLoc()
        mx,my = mx+parentX, my+parentY
        local mapLeft,mapTop=mx-mw*.5,my+mh*.5
        x,y = x-mapLeft,y-mapTop
        local bValid=true
        if y > 0 then
            bValid = false
        end
        x,y = x/mw,math.abs(y)/mh
        if x < 0 or x > 1 or y > 1 then
            bValid=false
        end
        x,y = DFMath.lerp(1, INFO_MAP_SIZE, DFMath.clamp(x, 0, 1)), DFMath.lerp(1, INFO_MAP_SIZE, DFMath.clamp(y, 0, 1))
        return x,y,bValid
    end
    ...
    function Ob:mapToWorld(x,y)
        ...
        x,y = x*mw/INFO_MAP_SIZE, y*mh/INFO_MAP_SIZE
        x,y = x+mapLeft, -y+mapTop
        return x,y
    --[[
    ...
    function Ob:setMapLoc()
        ...
        local sw,sh=g_GuiManager.getUIViewportSizeX(),g_GuiManager.getUIViewportSizeY()
        local leftMargin = self.rSidebarLeft:getDims() - 90
        local rightMargin = self.rSidebarRight:getLoc() - 20
        local rightWidth = self.rSidebarRight:getDims() - 20
        rightMargin = rightMargin+self.rSidebarRight:getAttrLink(MOAITransform.INHERIT_TRANSFORM):getLoc()
        local targetWidth = rightMargin-leftMargin
        
        -- for non-stretchy map
        --local scale = math.min(targetWidth/mapWidth,sh/mapHeight)
        --self.rMap:setScl(scale,scale)
        -- for stretchy map
        local sw,sh = targetWidth/mapWidth,sh/mapHeight
        self.rMap:setScl(sw,sh)
        
        self.rMap:setLoc((leftMargin-rightWidth)*.5,0)
        --local mapWidth, mapHeight = self.rMap:getDims()
        --self.rMap:setLoc(mapWidth*-.5,mapHeight*.5)
    --[[
        local mapWidth, mapHeight = self.rMap:getDims()
    ...
    function Ob:onResize()
        ...
        self.rTutorialMarker:setLoc(tutX-x-25,tutY-y+20)
        tutX,tutY = self:mapToWorld(12,34)
        self.rTutorialMarkerLabel:setLoc(tutX-x-5,tutY-y-35)
        ...
        if self.currentState == 'Deploying' then
            if self.bDoneDeploying then
                ...
                if self.nDeployTime > END_ANIM_INITIAL_DELAY then
                    ...
                    self.rFlavorTextA:setColor(r,g,b,1)
                    self.rFlavorTextB:setColor(r,g,b,1)
