...
m.tDistanceCodes = { 
    { "UIMISC042TEXT", 'far' }, -- very distant
    { "UIMISC013TEXT", 'far' }, -- distant
    { "UIMISC012TEXT", 'mid' }, -- proximal
    { "UIMISC041TEXT", 'close' }, -- close
    { "UIMISC011TEXT", 'close' }, -- very close
}
...
function m.getDistanceFromValue(nValue)
    ...
    local distanceText = g_LM.line(tChoice[1])
    local distanceColor = tChoice[2]
    return distanceText, distanceColor
end

function m.getGalaxyMapValue(x, y, key)
    local tValues = require('UI.Data.GalaxyData')[x..","..y]
    local nVal = tValues[key]
    
    return math.min(math.max(0,nVal),1)
end
